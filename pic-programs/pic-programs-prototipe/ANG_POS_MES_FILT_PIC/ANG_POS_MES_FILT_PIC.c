...
void interrupt()
{
     if(INTCON.F2)
     {
            ...
            if (aux==1) {MPU6050_Init();aux=0;}     // inicializacion del modulo MPU
            
            accel[1]=(MPU_Read(0x3B)<<8)+MPU_Read(0x3C);
            accel[3]=(MPU_Read(0x3F)<<8)+MPU_Read(0x40);
            ang=atan((accel[1]*1.0)/(accel[3]*1.0))*57.295779+90.0;    // calculo del angulo de inclinacion
            angf=3.88*(0.0375*ang+0.0440*ang_1+0.0474*ang_2+0.0474*ang_3+0.0440*ang_4+0.0375*ang_5);       //  aplicaicon del filtro digital
            ...
            delay_ms(1);
            PORTD.F0=0;
            //   actualizacion de variables utilizadas en el filtro
            ang_1=ang;
            ang_2=ang_1;
            ang_3=ang_2;
            ang_4=ang_3;
